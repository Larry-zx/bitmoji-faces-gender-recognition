...
class Classifier_Trainer(object):
    def __init__(self):
        ...
        self.device = torch.device("cuda:" + str(opt.gpu) if torch.cuda.is_available() else "cpu")  # 计算设备
    ...
    def set_model(self, model_type="convnext_base"):
        model = None
        if model_type == "convnext_base":
            model = convnext_base_one(pretrained=True, in_22k=True, num_classes=2)
